...
\subsubsection{Phase 2: Multi-Dimensional Transaction DP}

For a fixed route $R = [v_0, v_{i_1}, ..., v_{i_L}, v_0]$ of length $L+2$:

\textbf{State Representation}: $(i, c, \vec{\ell})$ where:
\begin{itemize}
    \item $i \in \{0, 1, ..., L\}$: Port index
    \item $c \in \mathbb{R}^+$: Capital level (may be discretized)
...
\textbf{Complexity}: For each route of length $L$: $O(L \times m \times k \times B^m \times K_{capital})$

\subsubsection{Overall Algorithm}

\textbf{Function} TwoPhaseHybridSolve(instance, timeout, beam\_width $K$):
\begin{enumerate}
    \item Phase 1: Generate promising routes using beam search
    \item Phase 2: For each route (until timeout):
    \begin{enumerate}
        \item Solve transaction optimization using DP
        \item Update best solution if improved
    \end{enumerate}
    \item Return best solution
\end{enumerate}

\textbf{Overall Complexity}: $O(K \times n^3 + K \times n \times m \times k \times B^m \times K_{capital})$
...
\textbf{Environment}: All tests were run on a standard desktop computer. Random seeds were fixed for reproducibility. Timeout parameter: 200 seconds.
...
Table~\ref{tab:performance} presents execution results for larger instances.
...
\textbf{Observations}:
\begin{itemize}
    \item Successfully handles instances up to $n=30$ within reasonable time (under 62 seconds)
    ...
    \item Execution time grows sub-exponentially with $n$
    \item Finds profitable solutions for all tested instances
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Efficient Algorithm}: Designed a two-phase hybrid algorithm combining beam search route generation with multi-dimensional dynamic programming, successfully handling instances up to $n=30$ within reasonable time.
...
\begin{itemize}
    ...
    \item The two-phase hybrid algorithm successfully addresses medium-to-large instances ($15 \leq n \leq 30$), achieving sub-exponential time growth and finding profitable solutions for all tested instances.
    
    \item For instances with $n > 11$, exact methods become impractical, justifying heuristic approaches. The two-phase hybrid algorithm provides an effective solution for these cases.
\end{itemize}
