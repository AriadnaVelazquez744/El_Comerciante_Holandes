...
\begin{abstract}
This document presents the design, implementation, and analysis of a two-phase hybrid algorithm for solving the Dutch Merchant Problem with instances of size $15 \leq n \leq 30$ ports, supporting multiple item types per port ($m$ items, with $m$ up to 15) and operations involving $k$ units per transaction. The algorithm separates route generation (using Prize-Collecting TSP-inspired heuristics) from transaction optimization (using adaptive multi-dimensional dynamic programming with critical efficiency improvements). Recent improvements enable the algorithm to complete in less than 10 minutes for instances with $n=30$ and $10 \leq m \leq 15$, which were previously intractable. The algorithm uses exact DP for small $m$ ($m < 6$), item-independent approximation for medium $m$ ($6 \leq m < 8$), and total load discretization for large $m$ ($m \geq 8$), achieving efficient solutions for medium-to-large instances while maintaining theoretical foundations in both PCTSP and DP literature. This work corresponds to the efficient algorithm implementation phase of the project, designed to handle larger instances than exact brute force methods can solve.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Phase 2 - Transaction Optimization}: For each generated route, uses adaptive dynamic programming to solve the transaction optimization problem. The algorithm adaptively selects the most appropriate solver based on $m$: exact multi-dimensional DP for $m < 6$, item-independent approximation for $6 \leq m < 8$, and total load discretization for $m \geq 8$. This exploits the optimal substructure: the best transaction decisions at port $i$ depend only on the current state (capital, load vector) and future ports, not on past decisions.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Transaction Optimization}: For each route, we solve the transaction problem using the most appropriate method: exact DP for small $m$ (maintaining optimality), or approximation methods for large $m$ (enabling feasibility). This guarantees that if the route generation includes the optimal route and $m < 6$, the overall solution will be optimal. For larger $m$, high-quality approximations are obtained efficiently.
...
\subsubsection{Adaptive Solver Selection}

The algorithm adaptively selects the most appropriate solver based on the number of item types $m$:

\textbf{Exact Multi-Dimensional DP ($m < 6$)}: Uses the full state space $(i, c, \vec{\ell})$ as described above. Maintains optimality guarantees.

\textbf{Item-Independent Approximation ($6 \leq m < 8$)}: For medium-sized $m$, the state space $O(B^m)$ becomes prohibitively large. The algorithm solves each item independently:
\begin{enumerate}
    \item Allocates capacity proportionally based on profit potential of each item
    \item Solves each item independently using greedy strategy
    \item Combines results from all items
\end{enumerate}
\textbf{Complexity}: Reduces from $O(B^m \times L)$ to $O(m \times B \times L)$ states. For $m=10$, $B=5$: reduction from $5^{10} = 9,765,625$ to $10 \times 5 = 50$ states per port (195,312$\times$ reduction).

\textbf{Total Load Discretization ($m \geq 8$)}: For very large $m$, tracks only total load instead of per-item loads:
\begin{enumerate}
    \item State space: $(i, c, \ell_{total}, t)$ where $\ell_{total} \in [0, B]$ is total load across all items
    \item Uses greedy action selection (do nothing, buy 1, sell 1 per item)
    \item Maintains approximate item distribution for decision reconstruction
\end{enumerate}
\textbf{Complexity}: Reduces to $O(B \times L)$ states (independent of $m$!). For $m=15$: enables feasibility where exact methods would be intractable.

\subsubsection{Optimization Strategies}

\begin{enumerate}
    \item \textbf{Capital Discretization}: For large capital ranges, discretize into bands of width $\Delta = \max(1.0, r / 100)$:
    ...
    \item \textbf{Dominance Pruning}: For state spaces $> 1000$ states, remove dominated states before travel phase. State $A$ dominates state $B$ if: same time used, $A$ has more capital, and $A$ has same or less load (component-wise). Provides 10-30\% additional state space reduction.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{For $n \geq 15$}: Single-pass route generation only (skip neighbor generation and exhaustive search)
        \item Allocate 50\% of timeout to route generation (instead of 30\%)
    ...
    \begin{itemize}
        ...
        \item Select solver based on $m = |\mathcal{M}|$:
        \begin{itemize}
            \item \textbf{If} $m < 6$: Use exact multi-dimensional DP
            \item \textbf{Else if} $m < 8$: Use item-independent approximation
            \item \textbf{Else}: Use total load discretization
        \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Phase 2 - Transaction DP}: Complexity depends on solver used:
    \begin{itemize}
        \item \textbf{For $m < 6$ (exact)}: $O(L \times m \times k \times B^m \times K_{capital})$ per route
        \item \textbf{For $6 \leq m < 8$ (item-independent)}: $O(L \times m \times B \times K_{capital})$ per route
        \item \textbf{For $m \geq 8$ (total load)}: $O(L \times B \times K_{capital})$ per route (independent of $m$!)
        \item With $K$ routes and $L = O(n)$: 
        \begin{itemize}
            \item Exact: $O(K \times n \times m \times k \times B^m \times K_{capital})$
            \item Item-independent: $O(K \times n \times m \times B \times K_{capital})$
            \item Total load: $O(K \times n \times B \times K_{capital})$
        \end{itemize}
    \end{itemize}
    
    \item \textbf{Overall Complexity}: Adaptive based on $m$:
    \begin{itemize}
        \item $m < 6$: $O(K \times n^3 + K \times n \times m \times k \times B^m \times K_{capital})$
        \item $6 \leq m < 8$: $O(K \times n^3 + K \times n \times m \times B \times K_{capital})$
        \item $m \geq 8$: $O(K \times n^3 + K \times n \times B \times K_{capital})$ (independent of $m$)
    \end{itemize}
    
    \item \textbf{Key Advantage}: Unlike brute force's $O(n! \times (k+1)^{m \cdot n})$, this is polynomial in $K$ (which we control) and adaptively reduces complexity based on $m$. For large $m$, the complexity becomes independent of the number of item types, enabling solution of instances with $m \geq 10$ that would be intractable with exact methods.
\end{itemize}
...
\subsection{Continuous Search Loop for Route Evaluation}

The route evaluation in the hybrid algorithm now uses a robust continuous search loop with batch cycle control, adaptive route generation, and best-solution tracking:

\textbf{Continuous Cycle and Batch Processing:}
\begin{itemize}
    \item Routes are evaluated in batches of 50. The loop continues until:
        \begin{itemize}
            \item A valid solution is found ($capital \geq initial\_capital$),
            \item 6 cycles are completed (max 300 routes),
            \item All routes are exhausted,
            \item Timeout is reached (default 480s)
        \end{itemize}
    \item Each batch only processes new (unseen) routes; previously evaluated routes use a cached result to avoid redundant DP calls.
    \item Results are cached (route tuple $\to$ (capital, decisions)) and reused for repeats.
    \item Non-trivial and higher-bound routes are prioritized; trivial ($[0,0]$) only at última instancia.
    \item Search stops immediately on finding a valid solution.
\end{itemize}

\textbf{Continuous and Adaptive Search:}
\begin{itemize}
    \item Old fallback rounds and forced stops are replaced by a continuous, adaptive loop:
        \begin{itemize}
            \item When all routes in the current batch are processed, more routes are generated as needed, up to 20s per generation.
            \item If many routes have been evaluated (e.g., $>100$), the system generates variations by adding unvisited ports to best found routes.
            \item The beam width may increase in extra generations (up to 500).
        \end{itemize}
    \item During route evaluation:
        \begin{itemize}
            \item Quick feasibility checks skip routes with $bound < initial\_capital$ or too low compared to best.
            \item The algorithm always tracks and returns the best solution found, even if invalid, with an $is\_valid$ flag (capital $< initial\_capital$ is permitted, but clearly indicated).
            \item No NaN is returned -- always outputs the best found.
        \end{itemize}
\end{itemize}

\textbf{Impact and Results:}
\begin{itemize}
    \item Maximizes the search effort within the permitted time, giving a much higher probability of finding a valid solution (not limited to a single batch of 50).
    \item Memory-efficient: the route evaluation cache remains small (even under 300 routes = $<1$MB).
    \item The process is efficient (avoids duplicate DP evaluations), early-stopping, and always delivers a meaningful result ($is\_valid$, $timeout$ indicators).
    \item The flow is robust: no más dependencias de que la solución esté en el primer batch, y no se omite ningún candidate prometedor.
\end{itemize}
...
The two-phase hybrid algorithm successfully addresses the challenge of solving the Dutch Merchant Problem for medium-to-large instances ($15 \leq n \leq 30$) with multiple items ($m$ up to 15) and $k$-unit operations. Key achievements:

\begin{enumerate}
    \item \textbf{Scalability}: Handles instances up to $n=30$ with $m=2$ within reasonable time (under 62 seconds), and instances with $n=30$, $m=10-15$ within 5-8 minutes (previously intractable)
    \item \textbf{Efficiency}: Reduces route exploration from $O(n!)$ to $O(K \times n)$ where $K$ is controlled. Single-pass route generation for $n \geq 15$ eliminates 20-50\% of execution time
    \item \textbf{Adaptive Optimality}: For each generated route, adaptively selects solver:
    \begin{itemize}
        \item Exact DP for $m < 6$: Maintains optimality guarantees
        \item Item-independent approximation for $6 \leq m < 8$: Near-optimal solutions with 195,312$\times$ state reduction for $m=10$
        \item Total load discretization for $m \geq 8$: High-quality approximations with complexity independent of $m$
    \end{itemize}
    \item \textbf{Flexibility}: Supports variable numbers of items ($m$) and operation sizes ($k$), with adaptive complexity reduction based on problem characteristics
    \item \textbf{Critical Efficiency Improvements}: Recent enhancements enable solving previously intractable instances:
    \begin{itemize}
        \item Route cycling elimination: Saves 2-3 minutes for large $n$
        \item Item-independent approximation: Enables feasibility for $m \geq 6$ (saves hours)
        \item Total load discretization: Additional 1-2 minutes saved for $m \geq 8$
...
The experimental evidence indicates that the two-phase hybrid algorithm exhibits markedly better scalability than exact brute-force methods. Execution times grow much more slowly than the factorial behaviour of exhaustive search, and the beam width parameter provides a tunable mechanism to balance solution quality against computational effort. For medium-sized instances in the range $15 \leq n \leq 30$ with $m=2$, the algorithm consistently produces profitable and feasible solutions within seconds to minutes, whereas exact methods become intractable.

With the critical efficiency improvements, the algorithm can now handle large instances with $n=30$ and $m=10-15$ within 5-8 minutes, which were previously impossible to solve (timeout $>10$ minutes). The adaptive solver selection ensures that:
\begin{itemize}
    \item Small instances ($m < 6$) maintain optimality through exact DP
    \item Medium instances ($6 \leq m < 8$) achieve near-optimal solutions efficiently
    \item Large instances ($m \geq 8$) become feasible through total load discretization
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{State Space Refinement}: Further reduce the DP state space for large $m$ or $B$ through item grouping or decomposition techniques. The current adaptive approach (exact DP, item-independent, total load) provides a solid foundation that can be extended.
    
    \item \textbf{Performance Tuning}: Fine-tune the thresholds for solver selection ($m < 6$, $6 \leq m < 8$, $m \geq 8$) based on empirical analysis of solution quality vs. execution time trade-offs.
\end{enumerate}
